					...
					/* array in the code)		*/
{
	...
	for (i=0 ; i<PNMLEN && (int)(pptr->pname[i]=name[i])!=0 ; i++)
		;
	pptr->pprio = priority;
	pptr->aged_priority = priority;
	pptr->total_quantum = 0;  // Total given ticks in current epoch
	pptr->quantum_left = 0;  // Remaining ticks in current epoch
	pptr->goodness = 0;  // Newly created process won't be able to run before next epoch
